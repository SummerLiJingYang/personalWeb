...
public final class ImageUtils {
	...
	/** 目标图片品质(取值范围: 0 - 100) */
	private static final int DEST_QUALITY = 80;
	 ...
	 *            目标高度
	 */
	public static void zoom(File srcFile, File destFile, int destWidth, int destHeight, int dest_quality) {
     ...
     */
    public static void abscut(String sourceFilePath,String targetFilePath, int x, int y, int destWidth,
            int destHeight) {
        try {
            File resource = new File(sourceFilePath);
            ...
            if (srcWidth >= destWidth && srcHeight >= destHeight) {
                ...
                g.dispose();
                ImageIO.write(tag, "JPEG", new File(targetFilePath));
                //删除源文件
                resource.delete();
            }
        } catch (Exception e) {
     ...
     * @param srcImgPath
     * @param outImgPath
     * @param quality   目标大小（k）
     * @return 是否压缩
     */
    public static boolean zipImage(String srcImgPath, String outImgPath, int outImgLength) {
    	try{
	    	File file = new File(srcImgPath);
	    	File outFile = new File(outImgPath);
	    	if(file.exists() && file.isFile() && !outFile.exists()){
	    		int dest_quality = (outImgLength*1024*100)/(int)file.length();
	    		if(dest_quality>=100){
	    			dest_quality=80;
	    		}
    			...
    			zoom(file, new File(outImgPath), old_w, old_h,dest_quality);
	    		return true;
	    	}
    	}catch (Exception e){
    		e.printStackTrace();
